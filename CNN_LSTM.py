...
class CNN_LSTM():
    def __init__(self):
        self.cnnModel = None
        self.lstmModel = None
        self.imageSequence = numpy.zeros((20,3))
    ...
    def predict(self,image):
        ...
        #cv2.waitKey(0)
        labels = ['Surgery', 'Transition', 'iKnife']
        toolLabels = ['Surgery', 'Transition', 'iKnife']
        ...
        CNNout = toolLabels[numpy.argmax(toolClassification)]
        print("CNN output: " + toolLabels[numpy.argmax(toolClassification)])
        ...
        LSTMout = labels[labelIndex]
        networkOutput = str(label) + str(taskClassification)
        print("LSTM output:" + LSTMout)
        return CNNout, LSTMout
